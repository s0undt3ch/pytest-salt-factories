...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        'markers',
        'skip_on_salt_system_install(reason=None): Marker to skip tests when testing against salt installed in the system.',
    )
...
def session_markers_loader(salt_factories):
    ...
    overrides = {'file_client': 'local', 'features': {'enable_slsvars_fixes': True}}
    factory = salt_factories.salt_minion_daemon(minion_id, overrides=overrides)
