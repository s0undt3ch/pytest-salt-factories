...
class EventListener:
    ...
    auth_event_handlers = attr.ib(
        init=False, repr=False, hash=False, factory=weakref.WeakValueDictionary
    )
    ...
    def __attrs_post_init__(self) -> None:
        ...
        self.address = 'tcp://127.0.0.1:{0}'.format(ports.get_unused_localhost_port())
    ...
    def _run(self) -> None:
        ...
        while self.running_event.is_set():
            ...
            try:
                ...
                log.debug(
                    '%s store size after event received: %d', self, len(self.store)
                )
            except Exception:
    ...
    def stop(self) -> None:
        ...
        if self.running_thread.is_alive():
            log.debug(
                '%s The running thread is still alive. Waiting a little longer...', self
            )
        ...
        if self.cleanup_thread.is_alive():
            log.debug(
                '%s The cleanup thread is still alive. Waiting a little longer...', self
            )
...
def event_listener(request: SubRequest) -> EventListener:
    ...
    return cast(
        EventListener,
        request.config.pluginmanager.get_plugin('saltfactories-event-listener'),
    )
...
def pytest_sessionstart(session: Session) -> None:
    ...
    """
    event_listener = session.config.pluginmanager.get_plugin(
        'saltfactories-event-listener'
    )
