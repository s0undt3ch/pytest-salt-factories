...
class SaltMaster(SaltDaemon):
    ...
    def __attrs_post_init__(self):
        ...
        if self.config.get('open_mode', False) is False:
            ...
            self.before_start(
                self.event_listener.register_auth_event_handler,
                self.id,
                auth_event_callback,
            )
            self.after_terminate(
    ...
    def default_config(
        ...
        if system_install is True:
            ...
            _defaults = {
                ...
                'tcp_master_pub_port': salt.config.DEFAULT_MASTER_OPTS[
                    'tcp_master_pub_port'
                ],
                'tcp_master_pull_port': salt.config.DEFAULT_MASTER_OPTS[
                    'tcp_master_pull_port'
                ],
                'tcp_master_publish_pull': salt.config.DEFAULT_MASTER_OPTS[
                    'tcp_master_publish_pull'
                ],
                'tcp_master_workers': salt.config.DEFAULT_MASTER_OPTS[
                    'tcp_master_workers'
                ],
                ...
                'file_roots': {'base': [str(state_tree_root)]},
    ...
    def salt_minion_daemon(self, minion_id, **kwargs):
        ...
        """
        return self.factories_manager.salt_minion_daemon(
            minion_id, master=self, **kwargs
        )
    ...
    def salt_proxy_minion_daemon(self, minion_id, **kwargs):
        ...
        """
        return self.factories_manager.salt_proxy_minion_daemon(
            minion_id, master=self, **kwargs
        )
    ...
    def salt_cloud_cli(
        ...
        **factory_class_kwargs
        ...
        config = factory_class.configure(
            self, self.id, root_dir=root_dir, defaults=defaults, overrides=overrides
        )
    ...
    def salt_client(
        self, functions_known_to_return_none=None, factory_class=client.LocalClient
    ):
        """
