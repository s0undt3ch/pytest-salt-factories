...
class SaltProxyMinion(SaltDaemon):
    ...
    def default_config(
        ...
        if system_install is True:
            ...
            _defaults = {
                ...
                or salt.config.DEFAULT_MINION_OPTS['master_port'],
    ...
    def load_config(cls, config_file, config):
        ...
        """
        return salt.config.proxy_config(
    ...
    def get_check_events(self):
        ...
        if not pytest_config.get('master-id'):
            log.warning(
                "Will not be able to check for start events for %s since it's missing the 'master-id' key in the 'pytest-%s' dictionary, or it's value is None.",
                self,
                self.config['__role'],
            )
        else:
