...
class SaltApi(SaltDaemon):
    ...
    def __attrs_post_init__(self):
        ...
        if 'rest_cherrypy' in self.config:
            ...
            raise pytest.UsageError(
                'The salt-master configuration for this salt-api instance does not seem to have any api properly configured.'
            )
    ...
    def _configure(
        ...
        raise pytest.UsageError(
            "The salt-api daemon is not configurable. It uses the salt-master config that it's attached to."
        )
    ...
    def load_config(cls, config_file, config):
        ...
        """
        raise pytest.UsageError(
            "The salt-api daemon does not have it's own config file. It uses the salt-master config that it's attached to."
        )
