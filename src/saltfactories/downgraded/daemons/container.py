...
class Container(BaseFactory):
    ...
    def start(self, *command, max_start_attempts=None, start_timeout=None):
        ...
        raise FactoryNotStarted(
            'The {} factory has failed to confirm running status after {} attempts, which took {:.2f} seconds({:.2f} seconds each)'.format(
                self,
                current_attempt - 1,
                time.time() - start_time,
                start_timeout or self.start_timeout,
    ...
    def terminate(self):
        ...
        self._terminate_result = ProcessResult(
    ...
    def run(self, *cmd, **kwargs):
        ...
        return ProcessResult(
            returncode=returncode, stdout=stdout, stderr=stderr, cmdline=cmd
        )
    ...
    def run_container_start_checks(self, started_at, timeout_at):
        ...
        if start_check_callbacks:
            log.error(
                'Failed to run container start check callbacks after %1.2f seconds for %s. Remaining container start check callbacks: %s',
        ...
        while time.time() <= timeout_at:
            ...
            if check_ports:
                for (
                    container_binding,
                    host_binding,
                ) in check_ports_mapping.copy().items():
            ...
            return False
        log.debug(
            'All listening ports checked for %s: %s', self, self.get_check_ports()
        )
        return True
        ...
        try:
            ...
            if self.skip_on_pull_failure:
                ...
                raise pytest.skip.Exception(
                    "Failed to pull docker image '{}': {}".format(self.image, exc),
                    **exc_kwargs,
                ) from exc
            raise
...
class SaltDaemon(Container, bases.SaltDaemon):
    ...
    def before_start(self, callback, *args, on_container=False, **kwargs):
        """
    ...
    def after_start(self, callback, *args, on_container=False, **kwargs):
        """
    ...
    def before_terminate(self, callback, *args, on_container=False, **kwargs):
        """
    ...
    def after_terminate(self, callback, *args, on_container=False, **kwargs):
        """
