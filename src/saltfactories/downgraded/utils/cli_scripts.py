        ...
        with script_path.open('w') as sfh:
            ...
            if inject_coverage and not code_dir:
                raise pytest.UsageError(
                    "The inject coverage code needs to know the code root to find the path to the '.coveragerc' file. Please pass 'code_dir'."
                )
            if inject_coverage:
                ...
                script_contents += (
                    ...
                    .format(
                        sitecustomize_dir=str(
                            pathlib.Path(__file__).resolve().parent / 'coverage'
                        )
                    )
                    .strip()
                    + '\n\n'
